...
The world is generated server-side and deterministically on `start_game`, then shipped to all clients via `world_init`. Code lives under [`server/app/domain/world/`](../server/app/domain/world/).
...
- `teacher_loop.py` (in `game/`) — drives the tick.
- `pathfind.py` — A* on the grid.
- `physics.py` — server-side collision.

Teacher roster: 25 portraits in `client/public/teachers/`. Each teacher has an ability (throw balls, hand out detention slips, freeze the floor, …) — see `game/handlers/ability_handler.py`.
...
When all tasks are complete, `game/handlers/quest_handler.py` unlocks the extraction in the atrium (glowing grate in the floor).
