...
const buildAquarium: Builder = () => {
  const g = group("aquarium");
  const stand = box(1.3, 0.55, 0.42, M(0x3a3a40));
  stand.position.set(0, 0.275, -0.21);
  g.add(stand);
  const waterMat = lambertMaterial(0x2a9fd8, "aquariumWater");
  waterMat.alpha = 0.55;
  const water = box(1.24, 0.4, 0.36, waterMat);
  water.position.set(0, 0.78, -0.21);
  g.add(water);
  const rim = box(1.3, 0.04, 0.42, M(0x202024));
  ...
  for (const [fx, fy] of [[-0.35, 0.72], [0.1, 0.84], [0.42, 0.76]] as const) {
    const fish = box(0.1, 0.05, 0.04, Basic(0xff8c2e));
...
const buildSkeleton: Builder = () => {
  const g = group("skeleton");
  const base = cylinder(0.25, 0.3, 0.06, 12, M(0x303034));
  base.position.y = 0.03;
  g.add(base);
  const pole = cylinder(0.02, 0.02, 1.7, 8, M(0x55555c));
  pole.position.y = 0.88;
  g.add(pole);
  const bone = M(0xe8e2d2);
  const skull = sphere(0.11, 10, bone);
  skull.position.set(0, 1.62, -0.06);
  g.add(skull);
  // Ribcage: stacked thin slabs narrowing downward.
  for (let i = 0; i < 4; i++) {
    const rib = box(0.34 - i * 0.04, 0.045, 0.16, bone);
    rib.position.set(0, 1.38 - i * 0.1, -0.06);
    g.add(rib);
  }
  const pelvis = box(0.26, 0.08, 0.14, bone);
  pelvis.position.set(0, 0.92, -0.06);
  g.add(pelvis);
  for (const side of [-1, 1]) {
    const arm = box(0.04, 0.55, 0.04, bone);
    arm.position.set(side * 0.22, 1.18, -0.06);
    g.add(arm);
    const leg = box(0.05, 0.7, 0.05, bone);
...
const buildPiano: Builder = () => {
  ...
  const body = box(1.6, 1.2, 0.45, dark);
  body.position.set(0, 0.6, 0.1);
  g.add(body);
  const keybed = box(1.5, 0.07, 0.28, dark);
  keybed.position.set(0, 0.78, -0.24);
  g.add(keybed);
  const keys = box(1.42, 0.03, 0.22, M(0xf2efe6));
  keys.position.set(0, 0.83, -0.24);
  g.add(keys);
  // Black-key strip suggested as one thin slab.
  const blacks = box(1.36, 0.035, 0.1, dark);
  blacks.position.set(0, 0.85, -0.29);
  g.add(blacks);
  for (const side of [-1, 1]) {
    const legM = box(0.08, 0.72, 0.08, dark);
...
const buildWaterDispenser: Builder = () => {
  const g = group("waterDispenser");
  const body = box(0.36, 1.0, 0.36, M(0xeceff1));
  body.position.set(0, 0.5, -0.18);
  g.add(body);
  const bottleMat = lambertMaterial(0x4db3e8, "waterDispenserBottle");
  bottleMat.alpha = 0.7;
  const bottle = cylinder(0.14, 0.16, 0.4, 12, bottleMat);
  bottle.position.set(0, 1.2, -0.18);
  g.add(bottle);
  const tap = box(0.1, 0.06, 0.08, M(0x303034));
...
const buildTrophyCase: Builder = () => {
  const g = group("trophyCase");
  const caseBox = box(1.3, 1.0, 0.32, M(0x4a3526));
  caseBox.position.set(0, 1.1, -0.16);
  g.add(caseBox);
  const glassMat = lambertMaterial(0xbfdce8, "trophyCaseGlass");
  glassMat.alpha = 0.3;
  const glass = box(1.2, 0.86, 0.02, glassMat);
  glass.position.set(0, 1.1, -0.33);
  g.add(glass);
  for (const [tx, scale] of [[-0.4, 0.8], [0, 1.1], [0.4, 0.9]] as const) {
    const cup = cylinder(0.05 * scale, 0.07 * scale, 0.16 * scale, 10, M(0xffd24a));
    cup.position.set(tx, 0.95 + 0.08 * scale, -0.24);
    g.add(cup);
    const ball = sphere(0.045 * scale, 8, M(0xffd24a));
...
const buildEasel: Builder = () => {
  ...
  for (const side of [-1, 1]) {
    const leg = box(0.04, 1.5, 0.04, wood);
    leg.position.set(side * 0.28, 0.73, 0);
    leg.rotation.z = side * -0.18;
    g.add(leg);
  }
  const backLeg = box(0.04, 1.45, 0.04, wood);
  backLeg.position.set(0, 0.71, 0.3);
  backLeg.rotation.x = 0.35;
  g.add(backLeg);
  const tray = box(0.6, 0.04, 0.08, wood);
  tray.position.set(0, 0.62, -0.08);
  g.add(tray);
  const canvas = box(0.56, 0.7, 0.02, M(0xf6f3ea));
  ...
  for (const [px, py, c] of [
    ...
    const daub = box(0.09, 0.07, 0.005, Basic(c));
