...
import { box, cone, cylinder, group, lambertMaterial } from "../../rendering/babylon";
...
const buildLabBench: Builder = () => {
  const g = group("labBench");
  const cupboard = box(1.9, 0.78, 0.9, M(0x6f6a60));
  cupboard.position.y = 0.39;
  g.add(cupboard);
  const top = box(2.0, 0.06, 1.0, M(STONE));
  top.position.y = 0.81;
  g.add(top);
  // Doors, so the base doesn't read as one flat slab.
  for (const dx of [-0.47, 0.47]) {
    const door = box(0.86, 0.66, 0.02, M(0x807a6e));
    door.position.set(dx, 0.4, 0.46);
    g.add(door);
    const handle = box(0.02, 0.12, 0.02, M(STEEL));
    handle.position.set(dx + 0.36, 0.4, 0.48);
    g.add(handle);
  }
  // Sink basin sunk into one end.
  const basin = box(0.42, 0.05, 0.34, M(0x4a4f57));
  basin.position.set(-0.68, 0.845, 0);
  g.add(basin);
  const tap = cylinder(0.018, 0.018, 0.26, 8, M(STEEL));
  tap.position.set(-0.68, 0.97, -0.16);
  g.add(tap);
  const spout = box(0.02, 0.02, 0.16, M(STEEL));
...
const buildFumeHood: Builder = () => {
  const g = group("fumeHood");
  const body = box(1.4, 2.1, 0.7, M(CABINET));
  ...
  const FRONT = -0.7;
  const chamber = box(1.24, 0.82, 0.04, M(0x1b1e23));
  chamber.position.set(0, 1.16, FRONT + 0.02);
  g.add(chamber);
  const sashMat = lambertMaterial(0x9fd8e8, "fumeHoodSash");
  sashMat.alpha = 0.35;
  const sash = box(1.24, 0.5, 0.03, sashMat);
  sash.position.set(0, 1.72, FRONT + 0.02);
  g.add(sash);
  const rail = box(1.28, 0.05, 0.06, M(STEEL));
  rail.position.set(0, 1.45, FRONT + 0.01);
  g.add(rail);
  // Strip light along the top of the opening — reads as "still running".
  const lamp = box(1.1, 0.04, 0.04, Basic(0xdff3ff));
  lamp.position.set(0, 1.56, FRONT + 0.03);
  g.add(lamp);
  // Sill jutting into the room under the opening.
  const worktop = box(1.36, 0.05, 0.14, M(STONE));
...
const buildChemicalShelf: Builder = () => {
  const g = group("chemicalShelf");
  const back = box(1.2, 1.5, 0.03, M(0x6b6257));
  back.position.set(0, 0.75, -0.015);
  g.add(back);
  for (const sx of [-0.585, 0.585]) {
    const upright = box(0.03, 1.5, 0.3, M(0x6b6257));
...
const buildBunsenBurner: Builder = () => {
  const g = group("bunsenBurner");
  const base = cylinder(0.09, 0.11, 0.03, 10, M(0x33363c));
  base.position.y = 0.015;
  g.add(base);
  const barrel = cylinder(0.028, 0.034, 0.22, 8, M(STEEL));
  barrel.position.y = 0.14;
  g.add(barrel);
  const flame = cone(0.035, 0.14, 8, Basic(0x4fb8ff));
  flame.position.y = 0.32;
  g.add(flame);
  const inner = cone(0.018, 0.07, 6, Basic(0xdff0ff));
  inner.position.y = 0.28;
  g.add(inner);
  // Gas hose trailing off the side.
  const hose = cylinder(0.012, 0.012, 0.2, 6, M(0x2a2a2e));
...
const buildEmergencyShower: Builder = () => {
  const g = group("emergencyShower");
  const pipe = cylinder(0.035, 0.035, 2.2, 8, M(0x2f7a3f));
  pipe.position.set(0, 1.1, -0.16);
  g.add(pipe);
  const arm = box(0.06, 0.06, 0.42, M(0x2f7a3f));
  arm.position.set(0, 2.16, 0.05);
  g.add(arm);
  const head = cylinder(0.24, 0.16, 0.08, 12, M(CABINET));
  head.position.set(0, 2.1, 0.24);
  g.add(head);
  // Triangular pull handle on a rod.
  const rod = cylinder(0.012, 0.012, 0.8, 6, M(STEEL));
  rod.position.set(0.18, 1.7, 0.2);
  g.add(rod);
  const handle = box(0.22, 0.03, 0.03, Basic(0xd8c23a));
  handle.position.set(0.18, 1.31, 0.2);
  g.add(handle);
  // Eyewash bowl lower down.
  const bowl = cylinder(0.16, 0.1, 0.07, 10, M(CABINET));
  bowl.position.set(0, 1.02, 0.1);
  g.add(bowl);
  const sign = box(0.3, 0.3, 0.02, Basic(0x2f7a3f));
