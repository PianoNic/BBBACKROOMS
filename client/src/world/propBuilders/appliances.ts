...
const buildFridge: Builder = () => {
  const g = group();
  const body = box(0.75, 1.75, 0.65, M(0xeeeeea));
  body.position.y = 0.875;
  g.add(body);
  const gap = box(0.76, 0.03, 0.66, M(0x404048));
  gap.position.y = 1.05;
  g.add(gap);
  for (const dy of [0.45, 1.45]) {
    const handle = box(0.03, 0.35, 0.04, M(0x808088));
    handle.position.set(0.30, dy, -0.34);
    g.add(handle);
  }
  const vent = box(0.5, 0.04, 0.02, M(0x303034));
...
const buildPrinter: Builder = () => {
  const g = group();
  const T = 0.75;
  const base = box(0.45, 0.22, 0.38, M(0x2a2a2e));
  base.position.y = T + 0.11;
  g.add(base);
  const top = box(0.47, 0.03, 0.40, M(0xc8c8cc));
  top.position.y = T + 0.225;
  g.add(top);
  const tray = box(0.38, 0.015, 0.14, M(0xf0f0ec));
  tray.position.set(0, T + 0.13, -0.13);
  g.add(tray);
  const slot = box(0.32, 0.015, 0.004, M(0x101012));
  slot.position.set(0, T + 0.21, -0.191);
  g.add(slot);
  const panel = plane(0.10, 0.05, Basic(0x4adef0));
...
const buildSideTable: Builder = () => {
  const g = group();
  const top = box(0.95, 0.04, 0.95, M(0x6a4a2a));
  top.position.y = 0.73;
  g.add(top);
  for (const [dx, dz] of [
    [0.42, 0.42], [-0.42, 0.42], [0.42, -0.42], [-0.42, -0.42],
  ] as const) {
    const leg = box(0.05, 0.71, 0.05, M(0x3a2a1a));
...
const buildSofa: Builder = () => {
  ...
  const base = box(1.85, 0.4, 0.85, fabric);
  base.position.y = 0.2;
  g.add(base);
  const back = box(1.85, 0.55, 0.18, fabric);
  back.position.set(0, 0.65, 0.33);
  g.add(back);
  for (const dx of [-0.6, 0.6]) {
    const seat = box(0.78, 0.12, 0.7, cushion);
    seat.position.set(dx, 0.46, -0.06);
    g.add(seat);
  }
  for (const dx of [-0.92, 0.92]) {
    const arm = box(0.18, 0.45, 0.8, fabric);
    arm.position.set(dx, 0.45, -0.02);
    g.add(arm);
  }
  for (const dx of [-0.85, 0.85]) {
    for (const dz of [-0.3, 0.3]) {
      const leg = box(0.06, 0.08, 0.06, M(0x303034));
