...
import { Group, basicMaterial, group, cylinder as bxCylinder } from "../../rendering/babylon";
...
export function toiletPaper(): Group {
  const g = group("toiletPaper");
...
export function wateringCan(): Group {
  ...
  const cavity = bxCylinder(0.11, 0.11, 0.16, 16, M(0x101810));
  ...
  const handle = halfTorus(0.07, 0.012, 4, 10, wcMat);
  handle.position.set(0, 0.09, 0);
  g.add(handle);
  const spout = bxCylinder(0.018, 0.028, 0.16, 6, wcMat);
...
export function hdd(): Group {
  ...
  const sataPinMat = basicMaterial(0xc8b060);
  ...
  for (const [cx, cz] of [[-sx, -sz], [sx, -sz], [-sx, sz], [sx, sz]] as const) {
    const screw = bxCylinder(0.008, 0.008, 0.003, 8, screwMat);
  ...
  const pin1 = box(0.028, 0.004, 0.002, sataPinMat);
  pin1.position.set(-0.04, connectorY, D / 2 + 0.006);
  g.add(pin1);
  const pin2 = box(0.044, 0.004, 0.002, sataPinMat);
