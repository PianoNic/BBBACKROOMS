...
import { Group, basicMaterial, box, cone, group } from "../rendering/babylon";
...
function downArrow(): Group {
  const g = group("downArrow");
  const mat = basicMaterial(0xffe66b);
  const shaft = box(0.12, 0.4, 0.12, mat);
  shaft.position.y = 0.18;
  g.add(shaft);
  // Cone tip is at +Y by default → rotate π so it points down.
  const head = cone(0.26, 0.36, 4, mat);
...
export function buildSpot(o: Objective, s: Spot, group: Group): SpotVisuals {
  ...
  if (!isArrow) {
    ...
    const scale = SCALE_BY_ITEM[o.item as string] ?? 1.7;
    inner.scaling.set(scale, scale, scale);
    if (o.item === "eye") inner.rotation.x = (20 * Math.PI) / 180;
  }
  const marker = new Group("questMarker");
  marker.add(inner);
  ...
  marker.metadata = { baseY, spinning: true };
  marker.visible = !s.done;
  group.add(marker);

  let extra: TransformNode | undefined;
  if (o.item === "sponge" && s.anchor_x != null && s.anchor_z != null) {
    const doodle = buildWhiteboardDoodle() as unknown as TransformNode;
