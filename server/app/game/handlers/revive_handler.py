...
class ReviveHandler:
    ...
    async def _push_progress(self, p: PlayerConn, progress: float) -> None:
        await self._broadcaster.send_safe(p, {"type": "revive_progress", "progress": progress})

    async def handle_revive_start(self, lobby: Lobby, me: PlayerConn, target_id: str) -> None:
        ...
        await self._push_progress(me, 0.0)

    async def handle_revive_cancel(self, lobby: Lobby, me: PlayerConn) -> None:
        if me.id in lobby.revives:
            lobby.revives.pop(me.id, None)
            await self._push_progress(me, -1.0)

    async def cancel_revives_for(self, lobby: Lobby, player_id: str) -> None:
        ...
        for rid in drop:
            ...
            if reviver:
                await self._push_progress(reviver, -1.0)

    async def tick_revives(self, lobby: Lobby, now: float, on_complete) -> None:
        ...
        for rid, rv in lobby.revives.items():
            ...
            await self._push_progress(reviver, progress)
