...
from app.game.handlers.noise_handler import CHAIR_THROW_RADIUS, NoiseHandler
...
class ChairHandler:
    ...
    def _chair_state_pkt(self, lobby: Lobby, chair_id: str) -> dict:
    ...
    def _player_holding(self, lobby: Lobby, player_id: str) -> str | None:
        for c in lobby.chairs.values():
            if c.held_by == player_id:
                return c.id
        return None

    async def handle_pickup(self, lobby: Lobby, me: PlayerConn, chair_id: str) -> None:
        ...
        if self._player_holding(lobby, me.id) is not None:
            return
        if not within_radius(me, chair, PICKUP_RADIUS):
            return
        chair.held_by = me.id
        await self._broadcaster.broadcast(lobby, self._chair_state_pkt(lobby, chair_id))

    async def handle_drop(self, lobby: Lobby, me: PlayerConn) -> None:
        cid = self._player_holding(lobby, me.id)
        if cid is None:
            return
        ...
        chair.yaw = me.yaw
        await self._broadcaster.broadcast(lobby, self._chair_state_pkt(lobby, cid))

    async def handle_throw(
        self, lobby: Lobby, me: PlayerConn, dir_x: float, dir_z: float,
    ) -> None:
        ...
        self._noise_handler.emit_noise(lobby, me.x, me.z, CHAIR_THROW_RADIUS)

    def _cell_blocks(self, cells: list[int], width: int, height: int, x: float, z: float) -> bool:
        cx = int(x / CELL_SIZE)
        cz = int(z / CELL_SIZE)
    ...
    async def tick_projectiles(self, lobby: Lobby, dt: float) -> None:
        ...
        for proj in lobby.chair_projectiles:
            ...
            for _ in range(steps):
                ...
                if hit_id is not None:
                    break
                # Wall hit.
                if self._cell_blocks(cells, gw, gh, nx, nz):
                    wall = True
                    break
                cx, cz = nx, nz
            ...
            if hit_id is not None or wall or timed_out:
                ...
                if chair is not None:
                    ...
                    await self._broadcaster.broadcast(lobby, self._chair_state_pkt(lobby, chair.id))
                continue
            proj.x, proj.z = nx, nz
            alive.append(proj)
        lobby.chair_projectiles = alive

    async def release_chairs_held_by(self, lobby: Lobby, player_id: str) -> None:
        ...
        for c in lobby.chairs.values():
            ...
            await self._broadcaster.broadcast(lobby, self._chair_state_pkt(lobby, c.id))

    async def push_teacher_stuns(self, lobby: Lobby, now: float) -> None:
        ...
        if not payload:
            return
        await self._broadcaster.broadcast(lobby, {"type": "teacher_stuns", "teachers": payload})
