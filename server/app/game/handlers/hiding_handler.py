...
class HidingHandler:
    def __init__(
        self, broadcaster: Broadcaster, noise_handler: NoiseHandler, chair_handler: ChairHandler,
    ) -> None:
    ...
    def _line_of_sight(
        self, cells: list[int], w: int, h: int,
    ...
    def _nearest_in_range(
        self, lobby: Lobby, x: float, z: float,
    ...
    async def handle_hide(self, lobby: Lobby, me: PlayerConn) -> None:
        if lobby.world is None:
            return
        if me.hidden_in is not None:
            await self._exit(lobby, me)
            return
        if not self._broadcaster.is_active(lobby, me):
            return
        ho, saw_occupied = self._nearest_in_range(lobby, me.x, me.z)
        if ho is None:
            if saw_occupied:
                await self._broadcaster.send_safe(me, {"type": "hide_denied", "reason": "occupied"})
            return
        grid = lobby.world.grid
        for t in lobby.teachers:
            if not within_radius_xz(t.x, t.z, me.x, me.z, SEEN_RADIUS):
                continue
            if self._line_of_sight(grid.cells, grid.width, grid.height, t.x, t.z, me.x, me.z):
                await self._broadcaster.send_safe(me, {"type": "hide_denied", "reason": "seen"})
                return
        ...
        await self._chair_handler.release_chairs_held_by(lobby, me.id)
        ho.occupied_by = me.id
        me.hidden_in = ho.id
        me.x, me.z = ho.x, ho.z
        self._noise_handler.emit_noise(lobby, ho.x, ho.z, HIDE_NOISE_RADIUS)
        await self._broadcaster.broadcast(lobby, {
            "type": "player_hidden", "id": me.id, "hidden": True, "x": ho.x, "z": ho.z,
        })

    async def _exit(self, lobby: Lobby, me: PlayerConn) -> None:
    ...
    def free_hideout_for(self, lobby: Lobby, player_id: str) -> None:
