...
class NoiseHandler:
    def _is_active(self, lobby: Lobby, p: PlayerConn) -> bool:
        return p.id not in lobby.dead and p.id not in lobby.extracted

    def emit_noise(self, lobby: Lobby, x: float, z: float, radius: float) -> None:
        lobby.noise_events.append((x, z, radius))

    def track_movement_noise(self, lobby: Lobby, me: PlayerConn) -> None:
        ...
        if me.last_move_t > 0.0 and 0.0 < dt < 1.0:
            ...
            if (
                ...
                and self._is_active(lobby, me)
            ):
                me.last_noise_t = now
                self.emit_noise(lobby, me.x, me.z, SPRINT_RADIUS)
        me.last_move_x, me.last_move_z, me.last_move_t = me.x, me.z, now

    def handle_voice_noise(self, lobby: Lobby, me: PlayerConn) -> None:
        """Client reports the mic picked up speech; position is server truth."""
        now = _time.monotonic()
        if now - me.last_voice_noise_t < VOICE_EMIT_INTERVAL_S:
            return
        if not self._is_active(lobby, me):
            return
        me.last_voice_noise_t = now
        self.emit_noise(lobby, me.x, me.z, VOICE_RADIUS)

    def assign_noise_to_teachers(self, lobby: Lobby, now: float) -> None:
