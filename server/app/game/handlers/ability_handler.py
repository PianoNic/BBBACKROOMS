...
Delayed effects + tuning constants live in `app.game.ability_effects`.
"""
...
from app.game.ability_effects import (
    AbilityEffects,
...
class AbilityHandler:
    def __init__(self, broadcaster: Broadcaster, ability_effects: AbilityEffects) -> None:
        self._broadcaster = broadcaster
        self._ability_effects = ability_effects

    def _pick_random_room_point(
        self, lobby: Lobby, rng: random.Random,
    ...
    async def _relock_random_laptop(self, lobby: Lobby, by_teacher: str) -> str | None:
        ...
        for obj in lobby.world.objectives:
            ...
            if lobby.phase == "escape":
                lobby.phase = "tasks"
                await self._broadcaster.broadcast(lobby, {"type": "phase_change", "phase": "tasks"})
    ...
    def _find_player(self, lobby: Lobby, pid: str) -> PlayerConn | None:
        return lobby.conns.get(pid)

    async def apply_ability_events(
        self, lobby: Lobby, events: list[AbilityEvent], rng: random.Random,
    ) -> None:
        ...
        for ev in events:
            ...
            if ev.ability == "potion_throw" and ev.target_id:
                tgt = self._find_player(lobby, ev.target_id)
                if tgt is None:
                    continue
                travel = POTION_TRAVEL_MS / 1000.0
                asyncio.create_task(self._ability_effects.delayed_puddle(
                ...
                asyncio.create_task(self._ability_effects.delayed_stun(lobby, tgt.id, STUN_DURATION, travel))
                pkt["payload"] = {
                    "targetX": tgt.x, "targetZ": tgt.z,
                ...
                if stun_s > 0:
                    asyncio.create_task(self._ability_effects.delayed_stun(lobby, tgt.id, stun_s, travel))
                if slow_s > 0:
                    asyncio.create_task(self._ability_effects.delayed_slow(lobby, tgt.id, slow_f, slow_s, travel))
                pkt["payload"] = {
                ...
                pt = self._pick_random_room_point(lobby, rng)
                ...
                await self._relock_random_laptop(lobby, ev.teacher_id)
            ...
            await self._broadcaster.broadcast(lobby, pkt)
