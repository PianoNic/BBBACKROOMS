...
`revive_handler.py` to keep concerns separate."""
...
class PickupHandler:
    ...
    async def send_inventory(self, p: PlayerConn) -> None:
        await self._broadcaster.send_safe(p, {
            "type": "inventory",
            "medkits": p.medkits,
            "potions": p.potions,
    ...
    async def handle_collect(self, lobby: Lobby, me: PlayerConn, pickup_id: str) -> None:
        ...
        await self._broadcaster.broadcast(lobby, {"type": "pickup_taken", "id": pickup_id, "by": me.id})
        await self.send_inventory(me)

    async def handle_use_potion(self, lobby: Lobby, me: PlayerConn) -> None:
        if not self._broadcaster.is_active(lobby, me):
            return
        if me.potions <= 0:
            return
    ...
    async def handle_use_goggles(self, lobby: Lobby, me: PlayerConn) -> None:
        ...
        await self._broadcaster.send_safe(me, {
            "type": "goggles_state",
