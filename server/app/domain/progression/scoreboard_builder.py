...
class ScoreboardBuilder:
    def __init__(self, reward_calculator: RewardCalculator, level_calculator: LevelCalculator) -> None:
        self._reward_calculator = reward_calculator
        self._level_calculator = level_calculator

    def _ensure_ended(self, lobby: Lobby) -> tuple[float, float]:
    ...
    def _survival_seconds(self, lobby: Lobby, p: PlayerConn, start: float, end: float) -> float:
    ...
    def build(self, lobby: Lobby, result: str) -> dict:
        """Shared per-player + team stats. `result` is "won" or "lost"."""
        start, end = self._ensure_ended(lobby)
        ...
        for p in lobby.conns.values():
            ...
            survival = max(0.0, self._survival_seconds(lobby, p, start, end))
            players.append({
    ...
    def compute_rewards(self, lobby: Lobby, result: str) -> dict[str, dict]:
        ...
        for p in lobby.conns.values():
            extracted = p.id in lobby.extracted
            survival_ms = int(max(0.0, self._survival_seconds(lobby, p, start, end)) * 1000)
            xp = self._reward_calculator.earned_xp(
            ...
            coins = self._reward_calculator.earned_coins(xp, won)
            ...
            level_before, _, _ = self._level_calculator.level_from_total(before_total)
            level_after, xp_into, xp_for_next = self._level_calculator.level_from_total(after_total)
            out[p.id] = {
                "xpEarned": xp,
                "coinsEarned": coins,
