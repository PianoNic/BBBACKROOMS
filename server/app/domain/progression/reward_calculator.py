...
No I/O and no app imports on purpose (same discipline as scoreboard_builder.py)
so it can be called from the scoreboard/endgame path without circular-import
risk and unit-tested in isolation. Tuning lives here so the economy is
...
class RewardCalculator:
    def survival_xp(self, survival_ms: int) -> int:
        return min(int(survival_ms // 1000 // SURVIVAL_UNIT_S), SURVIVAL_XP_CAP)

    def earned_xp(
        self, *, tasks: int, revives: int, items: int, stuns: int,
    ...
    def earned_coins(self, xp: int, won: bool) -> int:
